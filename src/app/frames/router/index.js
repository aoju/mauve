import Vue from 'vue';
import store from '@/app/frames/stores';
import Router from 'vue-router';
import Index from '@/app/widget/index';
    ...
    'routes': [
        {
            ...
            'name': 'member',
            'path': '/member',
            'component': resolve => require(['@/app/exports/layout/UserLayout'], resolve),
            'meta': {'model': 'Login'},
            'children': [
                {
                    'path': 'login',
                    'name': 'login',
                    'component': () => import(/* webpackChunkName: "user" */ '@/app/widget/member/login'),
                    'meta': {'model': 'Login'}
                },
                {
                    'path': 'register',
                    'name': 'register',
                    'component': () => import(/* webpackChunkName: "user" */ '@/app/widget/member/Register'),
                    'meta': {'model': 'Login'}
                },
                {
                    'path': 'forgot',
                    'name': 'forgot',
                    'component': () => import(/* webpackChunkName: "user" */ '@/app/widget/member/forgot'),
                    'meta': {'model': 'Login'}
                }
                // {
                //     path: 'register-result',
                //     name: 'registerResult',
                //     component: () => import(/* webpackChunkName: "user" */ '@/app/widget/member/RegisterResult')
                // }
            ]
        },
        {
            'name': 'install',
            'path': '/install',
            'component': resolve => require(['@/app/widget/error/install'], resolve),
            'meta': {'model': 'error'}
        },
        {
            'name': 'resetEmail',
            'path': '/reset/email',
            'component': resolve => require(['@/app/widget/reset/email'], resolve),
            'meta': {'model': 'error'}
        },
        {
            'name': '404',
            'path': '/404',
            'component': resolve => require(['@/app/widget/error/404'], resolve),
            'meta': {'model': 'error'}
        },
        {
            'name': '403',
            'path': '/403',
            'component': resolve => require(['@/app/widget/error/403'], resolve),
            'meta': {'model': 'error'}
        },
        {
            'name': '500',
            'path': '/500',
            'component': resolve => require(['@/app/widget/error/500'], resolve),
            'meta': {'model': 'error'}
        }
    ]
});
