<template>
    <div>
        <a-tabs :animated="false" class="task-tag-index" defaultActiveKey="1" tabPosition="left" v-model="tabKey">
            <a-tab-pane :key="taskTag.code" v-for="taskTag in taskTagList">
                <div slot="tab">
                    <div class="tag-item">
                        <a-badge :class="`badge-${taskTag.color}`"
                                 status="success"/>
                        {{taskTag.name}}
                    </div>
                </div>
                            ...
                            <div class="infos">
                                <a-list :loading="{spinning: loading,delay: 300}">
                                    ...
                                    <a-list-item :key="index" v-for="(item, index) in list">
                                        <a-list-item-meta>
                                            <div :class="{'done': item.done}" class="task-list" slot="title">
                                                <div>
                                                    ...
                                                    <router-link :to="`/project/space/task/${code}/detail/${item.code}`"
                                                        ...
                                                        <a-avatar class="m-r-sm" icon="user" size="small"
                                                                  v-else></a-avatar>
                                                        <span class="task-name m-r">{{ item.name }}</span>
                                                        <span :key="tag.code" class="muted flex m-r tag-name"
                                                              v-for="tag in item.tags"
                                                        >
                                                    <a-badge :class="`badge-${tag.tag.color} m-r-xs`"
                                                             status="success"/>
                                                   {{tag.tag.name}}
                                                </span>
                                                    </router-link>
                                                </div>
                                                <div :class="showTimeLabel(item.end_time)" class="label task-time"
                                                     v-if="item.end_time">
...
<script>
    ...
    import {del, list} from '../../frames/restapi/taskTag';
    import pagination from '../../shared/pagination';
    import {formatTaskTime} from '../../../assets/js/dateTime';
    import {getListByTaskTag} from '../../frames/restapi/task';
